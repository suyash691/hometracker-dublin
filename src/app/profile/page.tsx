    ...
    <div className="space-y-6">
      ...
      <form onSubmit={save} className="bg-white p-6 rounded-lg border space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div><label className="text-sm text-gray-500">Partner 1 Name</label><input required value={form.name1} onChange={e => setForm({ ...form, name1: e.target.value })} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
          <div><label className="text-sm text-gray-500">Partner 2 Name</label><input required value={form.name2} onChange={e => setForm({ ...form, name2: e.target.value })} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
          <div><label className="text-sm text-gray-500">Gross Income 1 (€)</label><input type="number" value={form.grossIncome1} onChange={e => setForm({ ...form, grossIncome1: e.target.value })} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
          <div><label className="text-sm text-gray-500">Gross Income 2 (€)</label><input type="number" value={form.grossIncome2} onChange={e => setForm({ ...form, grossIncome2: e.target.value })} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
          <div><label className="text-sm text-gray-500">Tax Paid 4yr — Partner 1 (€)</label><input type="number" value={form.taxPaid4Years1} onChange={e => setForm({ ...form, taxPaid4Years1: e.target.value })} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
          <div><label className="text-sm text-gray-500">Tax Paid 4yr — Partner 2 (€)</label><input type="number" value={form.taxPaid4Years2} onChange={e => setForm({ ...form, taxPaid4Years2: e.target.value })} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
          <div><label className="text-sm text-gray-500">Existing Monthly Debt (€)</label><input type="number" value={form.existingMonthlyDebt} onChange={e => setForm({ ...form, existingMonthlyDebt: e.target.value })} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
          <div><label className="text-sm text-gray-500">Total Savings (€)</label><input type="number" value={form.totalSavings} onChange={e => setForm({ ...form, totalSavings: e.target.value })} className="w-full border rounded px-3 py-2 text-sm mt-1" /></div>
          <div className="col-span-2"><label className="text-sm text-gray-500">Workplace Address</label><input value={form.workplaceAddress} onChange={e => setForm({ ...form, workplaceAddress: e.target.value })} className="w-full border rounded px-3 py-2 text-sm mt-1" placeholder="For commute estimates" /></div>
        </div>
