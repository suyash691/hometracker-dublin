        ...
        <div className="space-y-4">
          {!editNotes ? (<>
            ...
            <div className="flex items-center gap-3 text-sm">
              ...
              <input type="datetime-local" value={house.viewingDate ? new Date(house.viewingDate).toISOString().slice(0, 16) : ""} onChange={async (e) => { const v = e.target.value; await api.houses.update(id, { viewingDate: v ? new Date(v).toISOString() : undefined }); load(); }} className="border rounded px-2 py-1 text-sm" />
        ...
        <div className="space-y-4">
          <div className="flex gap-2 items-end">
            ...
            <button onClick={async () => {
              const json = prompt('Paste JSON array:\n[{"item":"Kitchen","estimatedCostLow":8000,"estimatedCostHigh":15000,"notes":"..."}]');
              if (!json) return;
              try { const data = JSON.parse(json); if (!Array.isArray(data)) throw new Error("Not an array"); await api.estimates.importJson(id, data); load(); }
              catch (err) { alert(err instanceof Error ? err.message : "Invalid JSON"); }
            }} className="bg-gray-600 text-white px-4 py-2 rounded text-sm">📋 Import JSON</button>
          </div>
