    ...
    <div className="space-y-6">
      ...
      <form onSubmit={save} className="bg-white p-6 rounded-lg border space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <label className="block"><span className="text-sm text-gray-500">Irish tax paid (up to last 4yr) — Partner 1 (€)</span><input type="number" value={form.taxPaid4Years1} onChange={e => setForm({ ...form, taxPaid4Years1: e.target.value })} className="w-full border rounded px-3 py-2 text-sm mt-1" /></label>
          <label className="block"><span className="text-sm text-gray-500">Irish tax paid (up to last 4yr) — Partner 2 (€)</span><input type="number" value={form.taxPaid4Years2} onChange={e => setForm({ ...form, taxPaid4Years2: e.target.value })} className="w-full border rounded px-3 py-2 text-sm mt-1" /></label>
