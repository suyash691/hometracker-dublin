...
export interface TotalCost { id: string; houseId: string; purchasePrice: number; deposit: number; stampDuty: number; legalFees: number; landRegistryFees: number; surveyFee: number; valuationFee: number; mortgageProtection: number; homeInsurance: number; movingCosts: number; otherCosts: number; cashNeededAtClosing?: number; totalPurchaseCost?: number; renovationLow?: number; renovationHigh?: number; seaiGrants?: number; trueAllInLow?: number; trueAllInHigh?: number; }
